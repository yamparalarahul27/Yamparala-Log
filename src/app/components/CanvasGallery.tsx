...
function SidePanel({
    ...
    return (
      <>
        ...
        <div
          ...
          className={`fixed inset-x-0 bottom-0 z-50 flex max-h-[85vh] flex-col overflow-hidden rounded-t-2xl border-t border-slate-200 bg-white/95 text-slate-900 backdrop-blur-2xl transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? "translate-y-0" : "translate-y-full"}`}
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex flex-col gap-4 px-5 pb-5 pt-3">
                <div>
                  ...
                  {item.description && <p className="mt-1.5 text-sm leading-relaxed text-slate-500">{item.description}</p>}
                </div>
                <PanelMeta item={item} />
                <a
                  href={item.url}
  ...
  return (
    <div className={`pointer-events-none fixed inset-y-0 right-0 z-50 flex w-[380px] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
      <div className="pointer-events-auto relative flex h-full w-full flex-col overflow-y-auto border-l border-slate-200 bg-white/90 text-slate-900 backdrop-blur-2xl">
        <button onClick={onClose} className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-colors hover:bg-slate-200">
          <X size={18} />
        </button>
        {item && (
          <>
            ...
            <div className="flex flex-1 flex-col gap-5 p-6">
              <div>
                ...
                {item.description && <p className="mt-2 text-sm leading-relaxed text-slate-500">{item.description}</p>}
              </div>
              <PanelMeta item={item} />
              <a
                href={item.url}
...
function HUD({ itemCount, isMobile }: { itemCount: number; isMobile: boolean }) {
  return (
    <div className="fixed left-3 top-[72px] z-40 flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white/80 px-2 py-1 text-[10px] font-medium text-slate-500 backdrop-blur-md sm:left-5 sm:gap-2 sm:px-3 sm:py-1.5 sm:text-xs">
